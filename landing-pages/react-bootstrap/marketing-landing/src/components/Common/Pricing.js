...
const Pricing = () => {
  ...
  const pricingPlans = [
    {
      ...
      tier: 1, // Used for conditional background styling
      isActive: false,
      features: [
    ...
    },
    {
      ...
      tier: 2, // Used for conditional background styling
      isActive: true,
      features: [
    ...
    },
    {
      ...
      tier: 3, // Used for conditional background styling
      isActive: false,
      features: [
        "Everything in Pro",
  ...
  // Function to determine background style based on plan tier
  const getBackgroundStyle = (tier) => {
    switch (tier) {
      case 1: // Free Trial
        return "linear-gradient(180deg, #FAF5FF 0%, #EEF6FF 100%)";
      case 2: // Pro
        return "linear-gradient(180deg, #FFF5ED 0%, #FAF5FF 100%)";
      case 3: // Enterprise
        return "linear-gradient(180deg, #EEF6FF 0%, #EEFFE5 100%)";
      default:
        return "linear-gradient(180deg, #F8F9FA 0%, #E9ECEF 100%)"; // Default fallback
    }
  };

  return (
    <>
      <div className="pricing-plans-marketing-area pb-180">
          ...
          <Row className="g-4 justify-content-center align-items-center">
            {pricingPlans.map((plan, index) => (
              <Col md={6} lg={4} key={`plan-${index}`}>
                <div
                  className={`pricing-plans-marketing-single-item position-relative z-1 ${
                    plan.isActive ? "active" : ""
                  }`}
                  style={{
                    background: getBackgroundStyle(plan.tier),
                  }}
                >
