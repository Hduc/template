...
<li class="header-right-item">
	<div class="dropdown notifications noti">
		<button
			class={`btn btn-secondary border-0 p-0 position-relative badge ${open ? 'show' : ''}`}
			type="button"
			on:click={() => (open = !open)}
		>
		...
		{#if open}
			<div
				class="dropdown-menu dropdown-lg p-0 border-0 p-0 dropdown-menu-end show"
			...
			>
				<div class="d-flex justify-content-between align-items-center title">
					<span class="fw-semibold fs-15 text-secondary"
						>Notifications
						<span class="fw-normal text-body fs-14">(03)</span></span
					>
					<button class="p-0 m-0 bg-transparent border-0 fs-14 text-primary"> Clear All </button>
				</div>

				<div class="max-h-217" style="overflow: overlay;">
					<div class="notification-menu">
						<a href="/dashboard/notification" class="dropdown-item">
