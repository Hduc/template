...
const UpcomingVIPReservations = () => {
  ...
  const [itemsPerPage] = useState<number>(5); // Set to 2 for demonstration; adjust as needed
  ...
  const totalItems: number = reservations.length;
  const totalPages: number = Math.ceil(totalItems / itemsPerPage);
  const indexOfLastItem: number = currentPage * itemsPerPage;
  const indexOfFirstItem: number = indexOfLastItem - itemsPerPage;
  const currentItems: Reservation[] = reservations.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
