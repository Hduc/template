<template>
    <v-card
        class="cash-at-end-of-the-month-card trezo-card border-radius d-block border-0 shadow-none"
    >
        <span class="d-block mb-3">Cash at End of the Month</span>
        <div style="margin: -37px 0 -5px 0">
            <div
                ref="cashAtEndOfTheMonthChart"
                style="width: 300px; height: 166px"
                class="m-auto"
            ></div>
        </div>
    </v-card>
</template>

<script>
import { defineComponent, onMounted, ref } from "vue";
import * as echarts from "echarts";

export default defineComponent({
    name: "CashAtEndOfTheMonthChart",
    setup() {
        const cashAtEndOfTheMonthChart = ref(null);

        onMounted(() => {
            if (cashAtEndOfTheMonthChart.value) {
                const chartInstance = echarts.init(
                    cashAtEndOfTheMonthChart.value
                );

                const options = {
                    tooltip: {
                        trigger: "item",
                    },
                    legend: {
                        bottom: "0",
                        left: "center",
                        itemWidth: 10,
                        itemHeight: 10,
                        textStyle: {
                            fontSize: 12,
                            color: "#64748B",
                        },
                    },
                    color: ["#37D80A", "#FD5812", "#605DFF"],
                    series: [
                        {
                            name: "Cash at End of the Month",
                            type: "pie",
                            radius: ["40%", "70%"],
                            center: ["50%", "70%"],
                            startAngle: 180,
                            endAngle: 360,
                            data: [
                                { value: 1048, name: "42.9%" },
                                { value: 735, name: "20.0%" },
                                { value: 580, name: "37.1%" },
                            ],
                        },
                    ],
                };

                chartInstance.setOption(options);
            }
        });

        return {
            cashAtEndOfTheMonthChart,
        };
    },
});
</script>
