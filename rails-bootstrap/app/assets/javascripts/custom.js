document.addEventListener("turbo:load", () => {
	(function () {
		...
		window.onload = function () {
			...
			// Header Sticky
			const getHeaderNavStickyId = document.getElementById(
				"for-header-nav-sticky"
			);
			if (getHeaderNavStickyId) {
				window.addEventListener("scroll", (event) => {
					...
					const { scrollTop } = event.target.scrollingElement;
					document
						.querySelector("#for-header-nav-sticky")
						.classList.toggle("sticky", scrollTop >= height);
				});
			}
		...
		const getHeaderBurgerMenuId =
			document.getElementById("header-burger-menu");
		if (getHeaderBurgerMenuId) {
			const switchtoggle = document.querySelector(".header-burger-menu");
			switchtoggle.addEventListener("click", function () {
				if (
					document.body.getAttribute("sidebar-data-theme") ===
					"sidebar-hide"
				) {
		...
		const getSidebarBurgerMenuId = document.getElementById(
			"sidebar-burger-menu"
		);
		...
		// Add Team Member JS
		document.addEventListener("DOMContentLoaded", function () {
			const AddTeamMemberContainer = document.getElementById(
				"AddTeamMemberContainer"
			);
			const AddTeamMemberInput =
				document.getElementById("AddTeamMemberInput");
			if (AddTeamMemberInput) {
				AddTeamMemberInput.addEventListener(
					"keydown",
					function (event) {
						if (
							event.key === "Enter" &&
							AddTeamMemberInput.value.trim() !== ""
						) {
		...
		try {
			var resultEl = document.querySelector(".resultSet"),
				plusMinusWidgets = document.querySelectorAll(
					".add-to-cart-counter"
				);
			...
			function changeHandler(event) {
				...
				for (var i = 0; i < plusMinusWidgets.length; i++) {
					resultEl.value =
						...
						Number(
							plusMinusWidgets[i].querySelector(".count").value
						);
				}
			}
		...
		try {
			...
			const getProductEditorId = document.getElementById(
			...
			const getMetaEditorId = document.getElementById(
		...
		if (getProjectManagementId) {
			...
			var dies_abr = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
			...
			function calendari(widget, data) {
				...
				var diff =
					data - new Date(original.getAttribute("data-actual"));
					...
					var countdownfunction = setInterval(function () {
						for (var i = 0; i < countDownDate.length; i++) {
							...
							if (distance < 0) {
								...
								countDownDate[i]["el"].querySelector(
									".days"
								).innerHTML = countDownDate[i]["days"];
								countDownDate[i]["el"].querySelector(
									".hours"
		...
		const getSalesByLocationsMapId = document.getElementById(
			"sales_by_locations_map"
		);
		...
		const getEnrolledByCountriesMapId = document.getElementById(
			"enrolled_by_countries_map"
		);
		...
		const getUsersByCountryMapId = document.getElementById(
			"users_by_country_map"
		);
		...
		const getWorldwideTopCreatorsMapId = document.getElementById(
			"worldwide_top_creators_map"
		);
		...
		const getSalesByCountryMapId = document.getElementById(
			"sales_by_country_map"
		);
		...
		const getMostSalesLocationId = document.getElementById(
			"most_sales_location"
		);
		...
		const getCallCenterGeographyId = document.getElementById(
			"call_center_geography"
		);
		...
		// Range Datepicker JS
		const getRangeDatepickerId =
			document.getElementById("range_datepicker");
		...
		try {
			...
			document
				...
				.addEventListener("click", nextTab);
			document
				.getElementById("prevButton")
				.addEventListener("click", previousTab);
		} catch {}
		...
		const getDigitalDateScheduleId = document.getElementById(
			"digital_date_schedule"
		);
		if (getDigitalDateScheduleId) {
			function updateDate() {
				...
				var month = monthNames[now.getMonth()];
				var year = now.getFullYear();
				var digitalDate = document.getElementById(
					"digital_date_schedule"
				);
		...
		const getSidebarToggleId =
			document.getElementById("sidebar-light-dark");
		...
		const getRadiusSquaresToggleId =
		...
		if (getCardBgToggleId) {
			...
			switchtoggle.addEventListener("click", function () {
				if (
					document.body.getAttribute("card-bg-data-theme") ===
					"card-bg-normal"
				) {
					document.body.setAttribute(
						"card-bg-data-theme",
		...
		document.querySelectorAll(".follow-button").forEach((button) => {
			...
			button.addEventListener("click", () => {
				...
				// Update button text
				followText.textContent = button.classList.contains("followed")
					? "Following"
					: "Follow";
			});
		});
					...
					if (audioElement.paused) {
						...
						audioPlayers.forEach((otherAudio, otherIndex) => {
							if (otherIndex !== index) {
								...
								otherAudio
									.closest("button")
									.querySelector(".play-pause").className =
									"play-pause play";
							}
						});
						audioElement.play();
		...
		if (geMenuLeftRightSlideId) {
			document.addEventListener("DOMContentLoaded", () => {
				...
				function updateMenu() {
					...
					nextBtn.disabled =
						currentIndex + itemsPerPage >= menuItems.length;
				}
		...
		const getWaitUntilTheDomIsFullyLoadedId = document.getElementById(
			"wait_until_the_dom_is_fully_loaded"
		);
			...
			document.addEventListener("DOMContentLoaded", function () {
				...
				calendarBody.addEventListener("click", function (event) {
					...
					if (event.target.tagName === "TD") {
						...
						const activeCell =
							calendarBody.querySelector(".active");
						if (activeCell) {
		...
		const getSendPrivateMassageId = document.getElementById(
			"send_private_massage"
		);
